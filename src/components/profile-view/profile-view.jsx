...
function ProfileView({ movies }) {
  ...
  const getProfileFunction = () => {
    ...
    fetch(`https://my-flix-sand.vercel.app/users/${parsedUser._id}`, {
  ...
  const updateUser = (event) => {
    ...
    fetch(`https://my-flix-sand.vercel.app/users/${parsedUser.userName}`, {
      method: 'PUT',
      headers: {
  ...
  const deleteAccount = () => {
    fetch('https://my-flix-sand.vercel.app/users/' + userName, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
